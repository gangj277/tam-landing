...
const rows = [
  { label: "졸업 기준", before: "204단위 (출석일수 기반)", after: "192학점 (학점 이수 기반)" },
  { label: "평가 체계", before: "9등급 상대평가", after: "5등급 + 성취도 A~E 병기" },
  { label: "시간표", before: "학교가 정한 고정 시간표", after: "학생이 직접 설계" },
  { label: "과목 수", before: "제한된 선택지", after: "138개 선택과목" },
  { label: "교실", before: "고정 교실", after: "과목별 이동수업" },
  { label: "미이수", before: "출석만 하면 졸업", after: "기준 미달 시 보충 이수 필요" },
];

export function CreditSystemComparison() {
  const { ref, isInView } = useInView(0.15);

  return (
    <section id="comparison" ref={ref} className="relative py-20 md:py-28 px-6">
