...
const upgrades: BoldUpgrade[] = [
  ...
  },
  {
    ...
    phrases: ["문제는 검사를 하는 데 있지 않습니다. 결과지를 아이의 정체성처럼 읽어버리는 순간부터 어긋나기 시작합니다."],
  },
  {
  ...
  },
  {
    ...
    phrases: ["넷째, 결과를 다음 경험 설계에 씁니다.", "결과지는 방향을 확정하는 도장이 아니라 다음 실험을 고르는 지도에 가깝습니다."],
  },
  {
...
async function main() {
  ...
  const editorFrame = await openUpdateEditor(page);
  ...
  for (const upgrade of upgrades) {
    await applyBoldUpgrade(editorFrame, upgrade);
  }
  ...
  await fs.writeFile(
    ...
    JSON.stringify(await captureArticleState(editorFrame), null, 2),
    "utf8",
  );

  const finalUrl = await publishUpdatedPost(page, editorFrame);
  await fs.writeFile(
...
async function openUpdateEditor(page: Page) {
...
async function waitForUpdateEditor(page: Page) {
...
async function dismissHelpOverlay(frame: Frame) {
...
async function applyBoldUpgrade(frame: Frame, upgrade: BoldUpgrade) {
  const result = await frame.evaluate(
    ({ paragraphIncludes, phrases }) => {
      ...
      const firstNode =
        paragraph.querySelector("span.__se-node") instanceof HTMLSpanElement
          ? (paragraph.querySelector("span.__se-node") as HTMLSpanElement)
          : null;

      const className = firstNode?.className || "se-ff-nanumgothic se-fs15 __se-node";
      const styleValue = firstNode?.getAttribute("style");
...
async function publishUpdatedPost(page: Page, frame: Frame) {
...
async function captureArticleState(frame: Frame) {
...
async function captureMobileScreenshot(browser: Browser, url: string) {
...
async function isVisible(locator: ReturnType<Frame["getByRole"]>) {
  try {
